...
export default {
	after (ctx, formatting, layout, canvas) {
		...
		layerCtx.save();
		layerCtx.drawImage(canvas, 0, 0, layout.canvas.width, layout.canvas.height);
		layerCtx.restore();

		const pixelData = layerCtx.getImageData(0, 0, layer.width, layer.height);
		const blurredPixelData = StackBlur.imageDataRGBA(pixelData, 0, 0, layer.width, layer.height, blur.radius);
